        ...
        if (exam["images"]) {
          for (var j = 0; j < exam["images"].length; j++) {
            ...
            if (image["image_laterality"]) {
              temp_row["Image Laterality"] = JSON.parse(
                JSON.stringify(image["image_laterality"])
              );
            } else {
              temp_row["Image Laterality"] = "None";
            }
            if (image["image_id"]) {
              temp_row["Image ID"] = JSON.parse(
                JSON.stringify(image["image_id"])
              );
            } else {
              temp_row["Image ID"] = "None";
            }
            if (image["image_procedure_id"]) {
              temp_row["Image Procedure"] = JSON.parse(
                JSON.stringify(image["image_procedure_id"])
              );
            } else {
        ...
        <div className="body_div_patient_images">
          ...
          <div className="table_design_patient_images">
            <Table
              ...
              submit_function={[
                ["Link To Image", this.props.submit_function, "view image"]
              ]}
            ></Table>
          </div>
        </div>
